...
func GetHistoryPurchase(c *gin.Context) {
...
func GetHistoryDeposit(c *gin.Context) {
	c.Header("Content-Type", "application/json")
	c.Header("Access-Control-Allow-Origin", "*")
	...
	for _, code := range dealer_code {
		var jsonData = []byte(`{
			"debitParty": "`+ code +`",
			"startDate": "`+ startDate.Format("2009-07-01 15:04:05") +`",
			"endDate": "`+ endDate.Format("2009-07-02 15:04:05") +`"
		}`)
